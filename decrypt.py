...
#     print(dna_to_bits(b_data, utils.dna_base_to_two_bits_table))
    print(type(dna_to_bits(b_data, utils.dna_base_to_two_bits_table)))
    print(len(dna_to_bits(b_data, utils.dna_base_to_two_bits_table)))
    
    return dna_to_bits(b_data, utils.dna_base_to_two_bits_table)


def main():
    de_blue, de_green, de_red = [], [], []
    
    original_file = np.load(original_filename)
    encrypted_text = np.load(encrypted_filename)
    ...
    for i in encrypted_text['b']:
        print(i)
        de_blue.append(dna_gdt(i, key))
    for i in encrypted_text['g']:
        print('GREEN')
        de_green.append(dna_gdt(i, key))
    for i in encrypted_text['r']:
        print('RED')
        de_red.append(dna_gdt(i, key))

#     print("Decrypted text:", decrypted_text)
#     decrypted_file.write(decrypted_text)
    np.savez(decrypted_filename, b = de_blue, g = de_green, r = de_red)
...
#     decrypted_file.close()
    ...
    decrypt = np.load(decrypted_filename)
    print(decrypt['b'])
    b_dec, g_dec, r_dec = dna_decode_2(decrypt['b'], decrypt['g'], decrypt['r'])
    print(b_dec, g_dec, r_dec)

    recover_image(r_dec, g_dec, b_dec, 'decrypt')
