...
def dna_get(text, key):
    ...
    b_data2 = text
    print("Initial DNA sequence:", text)
...
def main(filepath):
    global decryption_key
    global rounds_no
    blue_texts, green_texts, red_texts = [], [], []
    eblue_texts, egreen_texts, ered_texts = [], [], []
    
    # filepath = get_filepath()
#     print(filepath)

    blue, green, red = decompose_matrix(filepath)
    print(blue, green, red)
    print(len(blue), len(green), len(red))

    blue_binary, green_binary, red_binary = dna_encode(blue, green, red)
    print(blue_binary)
    print(len(blue_binary), len(blue_binary[0]))
   
    for row in blue_binary:
        blue_texts.append(''.join(row))
    for row in green_binary:
        green_texts.append(''.join(row))
    for row in red_binary:
        red_texts.append(''.join(row))
    

    m,n = blue_binary.shape

    # generate random key(it can have any length, could be the length of the plaintext)
    # Use 128 bit key in this case
    ...
    for blue_text in blue_texts:
        decryption_key = ''
        encrypted_text = dna_get(blue_text, key)
        eblue_texts.append(encrypted_text)
        print('After: ' + (encrypted_text))
        rounds_no = rrrr    
        print(rounds_no, decryption_key)
    ...
    for red_text in red_texts:
        decryption_key = ''
        encrypted_text = dna_get(red_text, key)
        ered_texts.append(encrypted_text)
        print('After: ' + (encrypted_text))
        rounds_no = rrrr 
        print(rounds_no, decryption_key)
    ...
    for green_text in green_texts:
        decryption_key = ''
        encrypted_text = dna_get(green_text, key)
        egreen_texts.append(encrypted_text)
        print('After: ' + (encrypted_text))
        rounds_no = rrrr
        print(rounds_no, decryption_key)
    ...
    print(len(eblue_texts), len(egreen_texts), len(ered_texts))
    
    np.savez(original_filename, b = blue_texts, g = green_texts, r = red_texts)
    np.savez(encrypted_filename, b = eblue_texts, g = egreen_texts, r = ered_texts)
    ...
    print('--- Starting recover image ---')
    
    encrypt = np.load(encrypted_filename)
    b_dec, g_dec, r_dec = dna_decode(encrypt['b'], encrypt['g'], encrypt['r'])
    recover_image(b_dec, g_dec, r_dec, 'encrypt')
    
    print('--- End of encrypting stage ---')
    
    
#     print('--- Decrypt Stage ---\n')
#     de_blue, de_green, de_red = [], [], []
#     for i in eblue_texts:
#         de_blue.append(dna_gdt(i, key)) 
#     for i in egreen_texts:
#         de_green.append(dna_gdt(i, key)) 
#     for i in ered_texts:
#         de_red.append(dna_gdt(i, key)) 
#     np.savez(decrypted_filename, b = blue_texts, g = green_texts, r = red_texts)
